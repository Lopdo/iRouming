...
public struct RefreshableList<Content: View>: View {
	...
	public var body: some View {
		...
		VStack {
			PullToRefreshView(data: self.data)
			content()
	...
	var body: some View {
		GeometryReader{ geometry in
			RefreshView(data: data)
				.opacity(Double((geometry.frame(in: .global).origin.y - 106) / 80)).preference(key: RefreshableKeyTypes.PrefKey.self, value: [RefreshableKeyTypes.PrefData(bounds: geometry.frame(in: .global))])
				.offset(y: -70)
		}
		.offset(y: -20)
	}
